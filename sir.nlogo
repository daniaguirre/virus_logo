...
to setup
  clear-all
  setup-nodes                               ;; crea los nodos
  setup-spatially-clustered-network         ;; conecta los nodos
  ask n-of initial-outbreak-size turtles    ;; identifica a los infectados iniciales
    [ become-infected ]
  ask links [ set color white ]             ;; pinta los enlaces de blanco
  reset-ticks
end
...
to setup-spatially-clustered-network
  ;; calcula el total de enlaces en la red
...
to go
  if all? turtles [not infected?]                      ;; termina cuando todas las tortugas están infectadas
    [ stop ]
...
to do-virus-checks
  ask turtles with [infected?] ;; para todas las tortugas infectadas que cubrieron su tiempo minimo de virus ...
  [
    if random 100 < recovery-chance                      ;; de acuerdo a la probabilidad de recuperacion, se recuperacion...
    [
      if random 100 < gain-resistance-chance         ;; elige si se vuelve resistente
        [ become-resistant ]
    ]
  ]
end
